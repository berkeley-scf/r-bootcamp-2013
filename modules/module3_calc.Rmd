...
What am I doing with `vals * (vals > 0)` ?
If you use a trick like this, having a comment in your code is a good idea.

# Linear algebra 

R can do essentially any linear algebra you need. It uses asystem-level packages called BLAS (basic linear algebra subroutines) and LAPACK (linear algebra package). Note that these calculations will be essentially as fast as if you wrote C code as R just calls C and Fortran routines to do the calculations.
...
See the R administration manual or talk to your systems manager. In the SCF we use something called *openBLAS* on the compute servers, *ACML* on the Linux cluster, and something called *vecLib* on the Macs.
...
And you may need to discretize a categorical variable, e.g., by age:
```{r fig.width=9}
...
ord[1:5]
earnings$earn[ord][c(1:5, 2025:2029)]
...
For each combination of sex by age group, collect the $R^2$ values from a regression of earnings on height, controlling for education, in a clean format, using tools such `sapply()`, `aggregate()`, or `by()`. Do not do this by looping. 
...
\[
\chi^2 = \sum_{i=1}^{n}\sum_{j=1}^{m} \frac{(y_{ij} - e_{ij})^2}{e_ij}, 
\] 
