...
Arrays are matrices but can have more or fewer than two dimensions.
```{r}
...
In general the placement of commas is crucial, but here, two different operations give the same result because of the underlying structure of data frames.
...
* Using the presidential preference dataset, create a new column based on *age9* that gives, as a numeric value, the midpoint of the age range assigned to each person. Do it both by using subsetting and by using string operations. 
